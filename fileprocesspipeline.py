...
from filepp.apdf2txt import pdf2txt,extract_images_from_pdf
...
class FileProcessPipeline():
    ...
    def process_text(self, filepath, doc_id, prompt_name, current_language, support_multi_modal=False, if_use_muilt=False, extract_text=False):
        text = None
        ...
        if filepath.endswith(".xlsx"):
            ...
            if support_multi_modal and if_use_muilt:
                print('多摩太处理中')
                images_output_path = os.path.join(os.path.dirname(filepath), Path(os.path.basename(filepath)).stem)
                try:
                    os.mkdir(images_output_path)
                except Exception as e:
                    print(e)
                images,nums,texts = extract_images_from_pdf(filepath, images_output_path,int(doc_id), get_text=True, extract_text=extract_text)
                for index, image in enumerate(images):
                    image_text = image2text_remote(image, prompt_name, current_language)
                    # 创建 Markdown 格式的文本
                    image_text = (
                        f"\n## image \n\n"  # 使用 Markdown 的标题语法
                        f"**image_name**: {os.path.basename(image)}\n"  # 使用加粗语法
                        f"**image description**: \n{image_text}\n"  # 使用加粗语法
                    )
                    texts[nums[index]] += image_text
                text = "\n".join(texts)
        ...
        elif support_multi_modal and if_use_muilt and (filepath.endswith(".jpg") or filepath.endswith(".jpeg") or filepath.endswith(".raw") or filepath.endswith(".png")) or filepath.endswith(".webp"):
            image_text = image2text_remote(filepath, prompt_name, current_language)
            text = (
                f"this document is generated by the image. \n"  # 使用 Markdown 的标题语法
                f"**image_name**: doc_{doc_id}_page_1_image_1_{os.path.basename(filepath)}\n"  # 使用加粗语法
                f"**image description**: \n{image_text}\n"  # 使用加粗语法
            )
    ...
    def process_file(self, filepath, doc_id, prompt_name, current_language, support_multi_modal=False, if_use_muilt=False, extract_text=False):
        outfile = None
        if DOC2PDF and filepath.endswith(".doc"):
            ...
            office_to_pdf(filepath, dst_path)
            filepath = dst_path
        text = self.process_text(filepath, doc_id, prompt_name, current_language, support_multi_modal=support_multi_modal, if_use_muilt=if_use_muilt, extract_text=extract_text)
        if text:
