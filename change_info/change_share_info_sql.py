...
parent_dir = Path(working_directory).parents[1]

print(parent_dir)

json_file_path = os.path.join(working_directory, "share_info.json")
...
cursor.execute('''INSERT INTO share_kdb_info (share_type, information) VALUES (?, ?)''', 
                ("share", information_json))
print("传送成功")
# 提交事务并关闭连接
conn.commit()
conn.close()
